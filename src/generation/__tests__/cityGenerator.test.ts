...
import { generateCity, SpatialIndex, ZONE_IDS, type GeneratedCity, type ZoneId } from '../index';
import type { Building, RoadKind } from '../types';

const VALID_ZONES: ReadonlySet<ZoneId> = new Set<ZoneId>(ZONE_IDS);

function buildCity(seed: number): GeneratedCity {
  return generateCity({ seed });
}

function deepEqualCity(a: GeneratedCity, b: GeneratedCity): boolean {
  if (a.seed !== b.seed) return false;
  if (a.width !== b.width) return false;
  if (a.height !== b.height) return false;
  if (a.roads.length !== b.roads.length) return false;
  if (a.zones.length !== b.zones.length) return false;
  if (a.companies.length !== b.companies.length) return false;
  if (a.buildings.length !== b.buildings.length) return false;
  for (let i = 0; i < a.roads.length; i++) {
    if (a.roads[i] !== b.roads[i]) return false;
  }
  ...
  for (let i = 0; i < a.companies.length; i++) {
    ...
    if (ca.zone !== cb.zone) return false;
    if (ca.buildingIds.length !== cb.buildingIds.length) return false;
    for (let j = 0; j < ca.buildingIds.length; j++) {
      if (ca.buildingIds[j] !== cb.buildingIds[j]) return false;
    }
  }
  for (let i = 0; i < a.buildings.length; i++) {
    ...
    if (ba.companyId !== bb.companyId) return false;
    if (ba.zone !== bb.zone) return false;
    if (ba.footprint.x !== bb.footprint.x) return false;
    if (ba.footprint.y !== bb.footprint.y) return false;
    if (ba.footprint.width !== bb.footprint.width) return false;
    if (ba.footprint.height !== bb.footprint.height) return false;
  }
  return true;
}
...
describe('CityGenerator', () => {
  ...
  test('road map distinguishes main and secondary roads', () => {
    ...
    let noneCount = 0;
    const kinds: ReadonlySet<RoadKind> = new Set<RoadKind>(['main', 'secondary', 'none']);
    for (const r of city.roads) {
      expect(kinds.has(r)).toBe(true);
      if (r === 'main') mainCount++;
      else if (r === 'secondary') secondaryCount++;
      else noneCount++;
    }
  ...
  test('exactly 5 zone ids exist and all non-road cells belong to one', () => {
    ...
    expect(ZONE_IDS.length).toBe(5);
    const observed = new Set<ZoneId>();
    for (let i = 0; i < city.zones.length; i++) {
      const z = city.zones[i];
      const r = city.roads[i];
      if (r === 'none') {
        expect(VALID_ZONES.has(z as ZoneId)).toBe(true);
        observed.add(z as ZoneId);
      } else {
  ...
  test('company count is between 8 and 12 inclusive, names unique and deterministic', () => {
    ...
    expect(city2.companies.map((c) => c.name)).toEqual(
      city.companies.map((c) => c.name),
    );
  });
  ...
  test('building footprints do not intersect main-road setback buffer', () => {
    ...
    for (let y = 0; y < city.height; y++) {
      for (let x = 0; x < city.width; x++) {
        if (city.roads[y * city.width + x] !== 'main') continue;
...
describe('SpatialIndex', () => {
  test('queryCell returns building indices whose footprint contains the cell', () => {
    ...
    const idx = new SpatialIndex(city);
  ...
  test('allBuildings returns the same buildings, in original order', () => {
    ...
    for (let i = 0; i < all.length; i++) {
      expect((all[i] as Building).id).toBe((city.buildings[i] as Building).id);
    }
  });
});
...
describe('Static-export compatibility', () => {
  test('generated source does not reference Next-only APIs', () => {
    ...
    expect(typeof generateCity).toBe('function');
