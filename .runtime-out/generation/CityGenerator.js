...
exports.ZONE_IDS = exports.SpatialIndex = exports.BuildingPlacer = exports.NameGenerator = void 0;
exports.generateCity = generateCity;
const NameGenerator_1 = require("./NameGenerator");
const BuildingPlacer_1 = require("./BuildingPlacer");
 ...
 *     (non-buildable).
 */
function carveRoads(grid, mainHorizontalY, mainVerticalX, secondaryStride) {
    const mainRoads = new Grid(grid.width, grid.height, false);
    ...
    for (let x = stride; x < grid.width; x += stride) {
        if (Math.abs(x - mainVerticalX) <= 1)
            continue;
        for (let y = 0; y < grid.height; y++) {
            if (grid.get(x, y) === 'main')
                continue;
            grid.set(x, y, 'secondary');
        }
    }
    for (let y = stride; y < grid.height; y += stride) {
        if (Math.abs(y - mainHorizontalY) <= 1)
            continue;
        for (let x = 0; x < grid.width; x++) {
            if (grid.get(x, y) === 'main')
                continue;
 ...
 */
function mainRoadBufferMask(width, height, mainRoads, setback) {
    const buf = new Grid(width, height, false);
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            if (!mainRoads.get(x, y))
                continue;
 ...
 */
function assignZones(width, height, roads, rng) {
    const zones = new Grid(width, height, 'residential');
    ...
    const w = (s) => 1 + Math.floor(rng() * 3); // 1..3
    const weight = {
        residential: 4,
        commercial: w(0) + 1,
        industrial: w(0),
    ...
    const order = ['residential', 'commercial', 'industrial', 'park', 'civic'];
...
function pickWeighted(rng, weight, bias) {
    const biased = {
    ...
    const total = types_1.ZONE_IDS.reduce((acc, z) => acc + (biased[z] ?? 0), 0);
    let r = rng() * total;
    for (const z of types_1.ZONE_IDS) {
        const w = biased[z] ?? 0;
        if (r < w)
            return z;
        r -= w;
 ...
 */
function enumerateZonePlots(width, height, zones, roads) {
    const out = new Map();
    for (const z of types_1.ZONE_IDS)
        out.set(z, []);
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            while (stack.length > 0) {
                ...
                if (seen.get(c.x, c.y))
                    continue;
                if (roads.get(c.x, c.y) !== 'none')
                    continue;
                if (zones.get(c.x, c.y) !== z)
                    continue;
...
function generateCity(options) {
    ...
    // Step 1: roads.
    const roads = new Grid(width, height, 'none');
    ...
    const nameGen = new NameGenerator_1.NameGenerator(options.seed);
    ...
    const zoneRotation = [...types_1.ZONE_IDS];
    ...
    const placer = new BuildingPlacer_1.BuildingPlacer({
        width,
        height,
        roads,
        zones,
    ...
    const zoneQueues = {
        residential: zoneIndices.residential.slice(),
        commercial: zoneIndices.commercial.slice(),
        industrial: zoneIndices.industrial.slice(),
        civic: zoneIndices.civic.slice(),
        park: zoneIndices.park.slice(),
    };
    ...
    const finalCompanies = companies.map((c, i) => ({
        id: c.id,
        name: c.name,
        zone: c.zone,
...
var NameGenerator_2 = require("./NameGenerator");
Object.defineProperty(exports, "NameGenerator", { enumerable: true, get: function () { return NameGenerator_2.NameGenerator; } });
var BuildingPlacer_2 = require("./BuildingPlacer");
Object.defineProperty(exports, "BuildingPlacer", { enumerable: true, get: function () { return BuildingPlacer_2.BuildingPlacer; } });
...
Object.defineProperty(exports, "ZONE_IDS", { enumerable: true, get: function () { return types_2.ZONE_IDS; } });
